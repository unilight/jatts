...
if __name__ == "__main__":
    ...
    for line in tqdm(train_all_files["segments"], desc="Reading segments"):
        ...
        if duration >= TRAIN_DURATION_MIN:
            item = {
                "sample_id": sample_id,
    ...
    print("train data size:", len(train_data))
    # assign prompt
    sample_ids_with_no_prompt = []
    for sample_id in tqdm(train_data, desc="Assigning prompt"):
        # Get all sample_ids for the same lecture_id
        candidate_sample_ids = train_spk2utt[train_data[sample_id]["lecture_id"]]
        # Filter candidates based on conditions
        valid_candidates = [
            sid for sid in candidate_sample_ids
            if sid in train_data and sid != sample_id and DURATION_MIN <= (train_data[sid]["end"] - train_data[sid]["start"]) <= DURATION_MAX
        ]
        # Randomly pick one valid candidate
        if valid_candidates:
            prompt_sample_id = random.choice(valid_candidates)
            train_data[sample_id]["prompt_sample_id"] = prompt_sample_id
            train_data[sample_id]["prompt_wav_path"] = train_data[prompt_sample_id]["wav_path"]
            train_data[sample_id]["prompt_start"] = train_data[prompt_sample_id]["start"]
            train_data[sample_id]["prompt_end"] = train_data[prompt_sample_id]["end"]
            train_data[sample_id]["prompt_original_text"] = train_data[prompt_sample_id]["original_text"]
            train_data[sample_id]["prompt_phonemes"] = train_data[prompt_sample_id]["phonemes"]
        else:
            sample_ids_with_no_prompt.append(sample_id)
    # remove samples with no prompt
    for sample_id in sample_ids_with_no_prompt:
        train_data.pop(sample_id, None)
    # Print total duration in hours
    total_duration = sum([item["end"] - item["start"] for item in train_data.values()])
    ...
    # assign prompt
    for sample_id in tqdm(dev_data, desc="Assigning prompt"):
        # Get all sample_ids for the same lecture_id
