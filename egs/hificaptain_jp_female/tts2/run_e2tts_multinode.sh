...
if [ "${stage}" -le 3 ] && [ "${stop_stage}" -ge 3 ]; then
    ...
    ######### MULTINODE TRAINING SETTING #########

    # Get all node IPs
    NODEFILE="${PJM_O_NODEINF}"
    ALL_IPS=($(cat $NODEFILE))
    MASTER_ADDR="${ALL_IPS[0]}"
    NUM_MACHINES=$(wc -l < "$NODEFILE")

    # Get my IP
    MY_IP=$(hostname -i)

    # Compute machine rank
    MACHINE_RANK=-1
    for i in "${!ALL_IPS[@]}"; do
        if [[ "${ALL_IPS[$i]}" == "$MY_IP" ]]; then
            MACHINE_RANK=$i
            break
        fi
    done
    ...
    # Safety check
    if [ "$MACHINE_RANK" -lt 0 ]; then
    echo "ERROR: Could not determine machine rank!"
    exit 1
    fi
    ...
    export PYTORCH_DIST_DEBUG=DETAIL
    export MASTER_ADDR="$MASTER_ADDR"
    export MASTER_PORT=29500

    echo "[INFO] Main node IP: $MASTER_ADDR"
    echo "[INFO] This machine rank: $MACHINE_RANK"
    echo "[INFO] Num machines: $NUM_MACHINES"

    # Only machine 0 generates config
    ACCELERATE_CONFIG="${expdir}/accelerate_config.yaml"
    if [ "$MACHINE_RANK" -eq 0 ]; then
        echo "Generating accelerate config from machine 0"
        python utils/generate_accelerate_config.py \
            --config_path "${ACCELERATE_CONFIG}" \
        ...
        echo "Waiting for accelerate config to be generated..."
        while [ ! -f "${ACCELERATE_CONFIG}" ]; do
            sleep 1
        done
    fi

    env | grep -E 'MASTER|IFNAME|FAMILY'

    ######### MULTINODE TRAINING SETTING ENDS #########
    
    log "Training start. See the progress via ${expdir}/train.log."
    VENV_PYTHON="$MAIN_ROOT/tools/venv/bin/python"
    ${cuda_cmd} --gpu "${n_gpus}" "${expdir}/train.log" \
        $VENV_PYTHON -m accelerate.commands.launch --config_file "${ACCELERATE_CONFIG}" ../../../jatts/bin/e2tts_train.py \
            --config "${conf}" \
