...
# from jatts.text.build_tokenizer import build_tokenizer
# from jatts.text.cleaner import TextCleaner
# from jatts.text.phoneme_tokenizer import g2p_choices
from jatts.utils.types import str2bool, str_or_none
...
def get_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--out", required=True, help="output token list path")
    parser.add_argument(
        "--column",
        required=True,
        help="which column of the input csv file to use",
    )
    parser.add_argument(
        "--token_type",
        default="phn",
        choices=["phn"],  # (TODO): add more token types in the future
        help="Token type",
    )
...
def main(cmd=None):
    ...
    counter = Counter()
    for line in dataset:
        if args.token_type == "phn":
            phonemes = line[args.column].split(" ")
            for t in phonemes:
                counter[t] += 1
    ...
    words_and_counts = list(
        filter(
            lambda x: x[1] > args.cutoff, sorted(counter.items(), key=lambda x: -x[1])
        )
    )

    # Parse the values of --add_symbol
    for symbol_and_id in args.add_symbol:
        # e.g symbol="<blank>:0"
        try:
            symbol, idx = symbol_and_id.split(":")
            idx = int(idx)
