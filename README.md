...
本リポジトリは研究目的に特化したオープンソース日本語テキスト音声合成（text-to-speech, TTS）ツールキットです。最初の目的としては，**異なるモデルの性能比較**なので，推論速度などの最適化は現時点まだ行っていません。

> レシピやモデルの拡張に興味ある方はぜひissueやPRなどで一緒に開発してください！

## インストール

ESPnetと同じく，本リポジトリは学習の時に必要な環境を事前に構築し，学習や評価などの指令を行うたびに，その環境を自動的に起動する，という形になります。環境の構築は以下の指令に従ってください。
...
__ドキュメントに不備や不明なところはぜひissueで投稿してください！メールの問い合わせも歓迎です。__
